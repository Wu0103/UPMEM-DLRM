...
class DLRM_Net(nn.Module):
    ...
    def export_emb(self, emb_l):
        ...
        for k in range(0, len(emb_l)):
            ...
            data_pointer=(c_int32)(emb_l[k].weight.data.data_ptr())
            nr_rows = 104857600
            ...
            #nr_rows=len(tmp_emb)
            #emb_data = np.round(tmp_emb).astype(int).flatten().tolist()
            #data_pointer=(c_int32*(len(emb_data)))(*emb_data)
            '''
            nr_rows=10000000
            data_pointer=(c_int32)(nr_rows)
            runtimes = pointer(DpuRuntimeTotals())
            my_functions.populate_mram(k,nr_rows,data_pointer,runtimes)
            '''
    ...
    def create_emb(self, m, ln, weighted_pooling=None):
        ...
        for i in range(0, ln.size):
            ...
            if self.qr_flag and n > self.qr_threshold:
                ...
                #EE.weight.data = torch.normal(mean=0.,std=1008611.,size=(n,m)) 
    ...
    def apply_emb(self, lS_o, lS_i, emb_l, v_W_l):
        ...
        def dpu_lookup(sparse_offset_group_batch, sparse_index_group_batch, k):
            ...
            my_functions.lookup(indices_pointer,offsets_pointer,indices_len,offsets_len,lookup_results,k)
            return lookup_results

        lr=[]
        my_functions.lookup.argtypes = POINTER(c_uint32), POINTER(c_uint32), c_uint64,c_uint64,POINTER(c_float), c_uint32
        my_functions.lookup.restype= None
        
        lS_o_1d = lS_o.flatten()
        lS_i_1d = torch.cat(lS_i)
        '''
        if lS_i_1d.size()[0]!=len(lS_i)*1024:
            lS_add = torch.tensor([0])
            for i in range(len(lS_i)*1024-1-lS_i_1d.size()[0]):
                lS_add = torch.cat((lS_add, torch.tensor([i])))
            lS_i_1d = torch.cat((lS_add, lS_i_1d))
        '''
        ts = time.time()
        ...
        result = dpu_lookup(lS_o_1d,lS_i_1d,len(lS_i))
        ...
        result_tensor = result_tensor.view(-1, args.mini_batch_size, self.m_spa)
        result_tensor.requires_grad = True
        for tensor in result_tensor:
            lr.append(tensor)
        '''
        for i in range(0, len(result), args.mini_batch_size * self.m_spa):
            data = result[i: i + args.mini_batch_size * self.m_spa]
            tensor = torch.tensor(data).reshape(args.mini_batch_size, self.m_spa)
