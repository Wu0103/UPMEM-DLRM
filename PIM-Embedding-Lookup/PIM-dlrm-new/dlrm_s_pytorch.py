    ...
    def create_emb(self, m, ln, weighted_pooling=None):
        ...
        for i in range(0, ln.size):
            ...
            if self.qr_flag and n > self.qr_threshold:
                ...
                #W = np.random.uniform(low=-np.sqrt((i+1) / n), high=np.sqrt((i+1) / n), size=(n, m)).astype(np.float32)
                #W = np.random.randint(-1008611, 1008611, size=(n, m),dtype=np.int32).astype(np.float32)

                #W = np.random.exponential(np.sqrt(1 / n), size=(n, m)).astype(np.float32)
                # approach 1
                #EE.weight.data = torch.tensor(W, requires_grad=True)
                EE.weight.data = torch.normal(mean=0.,std=1008611.,size=(n,m)) #torch.tensor(W, requires_grad=True)
